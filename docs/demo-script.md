...
Reference runtime: **about 1:48**. The source-bound capture report is authoritative. The recording is an edited product walkthrough: it removes inactive local-model wait time but does not synthesize a result or hide any user decision.
...
Show the archived YouTube and Bluesky **before** captures from the dummy accounts, then move directly to **Tune my feed**. Enter the conversational request:
...
Return to **Tune my feed**, replace the input sample with the six after-links, and compare. Keep the three YouTube and three Bluesky cards visible in both columns. Show the archived YouTube and Bluesky **after** captures from the same dummy accounts. Continue through **Copy feed**, **Incognito**, and **Blend**, then return to the passport cover.
...
| --- | --- | --- |
| 0:00–0:14 | Open on Curate, then show the real YouTube and Bluesky dummy-account home feeds marked **BEFORE**. | State the problem, audience, and portable-passport idea. |
...
| 0:28–0:45 | Show the six-link YouTube and Bluesky sample, then both Curate feed columns. | Describe the selected sample and visible content shift. |
...
| 0:58–1:12 | Show the real YouTube and Bluesky dummy-account home feeds marked **AFTER**. | Point out the visibly different recommendations without claiming the screenshots alone prove causality. |
| 1:12–1:25 | Preview **Copy feed** from YouTube to Bluesky and show translation loss. | Explain what carries over and what needs another route. |
| 1:25–1:36 | Issue and revoke **Incognito**. | Explain temporary context and expiry. |
| 1:36–1:42 | Show **Blend**. | Explain selected shared tastes without merging accounts. |
| 1:42–1:48 | End on the Curate passport. | Close with the product promise. |

## Recording boundary

The active product flow is local and account-free. It uses a real loopback model proposal, deterministic policy checks, a local practice feed, measurement, and rollback. The recorder does not log into or change YouTube, Bluesky, or Instagram. It presents owner-captured screenshots from previously reviewed dummy-account home feeds as archived evidence and binds every image to the recording report by SHA-256.
