...
| Live AgentCore deployment | Prepared but not deployed; optional to the complete local proof | Create the fixed USD 5 alert budget first, perform at most one managed `health` and one `plan_feed` invocation, retain redacted receipts, and tear down in the same session. Never imply credits or Budgets guarantee a hard cap |
