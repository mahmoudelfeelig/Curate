...
| Bluesky | Guided | Implemented live candidate for bounded account controls | The official OAuth/DPoP sidecar and opaque Python bridge cover follow/unfollow, actor mute/unmute, and muted-word add/remove. The owner-bound Connected Agent route can prioritize certified families and seal exact targets, while generic migrations cannot execute the live adapter. Loading requires publicly reachable HTTPS OAuth metadata/callbacks, an authorized dummy account, and a fresh signed conformance receipt for the exact revision; a Quick Tunnel is test-only, not durable production hosting. Custom feeds and ranking state are not copied. |
...
| YouTube | Guided | Implemented live candidate for subscriptions | Owner-bound subscription list/insert/delete maps to subscribe/unsubscribe. The owner-bound Connected Agent route can prioritize certified families and seal exact targets, while generic migrations cannot execute the live adapter. The public checkout bundles no signed live receipt; an exact-revision local receipt may temporarily elevate the runtime after authorized dummy-account conformance. Home ranking, watch history, and native recommendation feedback are neither read nor written through this adapter. |
...
Every unconfigured external adapter intentionally returns a `guided` runtime manifest even where official documentation or a dormant transport describes a stronger candidate surface. `documented_capabilities()`, transport source, OAuth registration, and a selected UI card cannot authorize execution. A runtime may load only the exact action subset in a fresh signature-, expiry-, revision-, owner-, dummy-account-, and provider-approval-validated conformance receipt. Generic migration authority still rejects live-adapter execution; the separately bound owner-authenticated Connected Agent API and desk are the only implemented live authority, and they fail closed without a certified adapter, sealed plan, and one-time owner approval. Test accounts do not waive platform rules.
