...
| AgentCore | Optional; a live demo and/or AgentCore deployment strengthens Technical Implementation | The checkout has a proposal-only Strands/Bedrock Runtime package plus plan-first CDK. It exposes strict `health`/`plan_feature`/`plan_feed` commands and no mutation authority. Managed evidence is kept as a redacted local exact-revision receipt, separate from source-test claims |
...
| Video | Public YouTube or Vimeo; maximum five minutes; working demo plus problem, user, and importance | A verified 62.891-second silent cut and matching short voiceover script are ready; record/mix the supplied voiceover, then publish the narrated cut |
...
| Presentation | 20% | The 62.891-second cut opens on the product, demonstrates both conversational and exact feed requests, shows a local-model plan, presents six visible before/after cards, and closes on verified rollback; the custom-domain site and judge login are live | Narration, public YouTube/Vimeo upload, Devpost screenshots, and final submission copy remain entrant-owned finishing work |
...
| Silent demo video | Ready: 62.891 seconds, 1440×900, no audio track, SHA-256 `22f3df13821771ce0625a0fe8ff7e20513592aa2ddba09dd4a813216c0b846cf`; an independent eight-frame audit confirms the visible vague request, exact request, six-card before/after comparison, and rollback route | Add the recorded voiceover when supplied, then upload the final narrated cut to YouTube or Vimeo |
