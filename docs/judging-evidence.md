...
| Repository | Public GitHub, GitLab, or Bitbucket repository with source, assets, setup instructions, and README | [Public GitHub repository](https://github.com/mahmoudelfeelig/curate) exists; recheck visibility and final source from a signed-out browser before submission |
...
| Video | Public YouTube or Vimeo; maximum five minutes; working demo plus problem, user, and importance | An approximately 217-second clean-revision tutorial cut and matching voiceover script show the complete before/request/agent/result flow; record or supply the voiceover, then publish the narrated cut |
...
| Presentation | 20% | The approximately 217-second tutorial pans 930 pixels through archived real YouTube and Bluesky dummy-account feeds, demonstrates conversational and exact requests, replays nine retained AWS execution events, tours twelve large practice-feed cards covering the requested subjects, measures a 100-point calm-content shift, places real feeds side by side before/after, and completes Copy Feed, Incognito, and two-person Blend flows; the custom-domain site and judge login are live | Narration, public YouTube/Vimeo upload, Devpost screenshots, and final submission copy remain entrant-owned finishing work |
...
| Silent demo video | The final 1440×900 cut binds a clean Git revision and evidence digest, checks both prompt styles, verifies four 930-pixel platform tours, two direct before/after comparisons, twelve practice-feed cards, seven exact topics totaling 100%, a 100-point ragebait reduction, Copy Feed, Incognito issue/revoke, Blend invitation/activation/stop, a nine-event managed-proof interstitial, and rollback, while rejecting external browser requests | Add the recorded voiceover when supplied, then upload the final narrated cut to YouTube or Vimeo |
