  ...
  return (
    <section className="passport-book section-book">
      ...
      <article className="passport-page left-page">
        ...
        {proposalPrefill ? <aside className="proposal-prefill-note" data-proposal-prefill="migration"><StatusStamp tone="blue" compact>PROPOSAL PREFILLED</StatusStamp><div><b>{proposalPrefill.goal}</b><p>{proposalPrefill.rationale}</p><small>No preview, migration, or receipt was created by the Feature Clerk.</small></div></aside> : null}
      ...
      <article className="passport-page right-page">
        ...
        {!preview ? <div className="empty-manifest"><b>NO PREVIEW STAMPED</b><p>Select a route and ask the agent to compile an action-and-loss manifest.</p></div> : (
          <>
            ...
            <div className="approval-block"><p><b>{actionTotal}</b> previewed actions · <b>{guidedTotal}</b> guided steps · <b>{lossTotal}</b> material translation limits{guidedPreview.requiresExactReview ? <><br /><span>The handoff records only the exact targets and instructions shown above. It performs zero API writes.</span></> : null}</p><ActionButton onClick={onApply} busy={busyAction === "migration-apply"} disabled={Boolean(outcome) || !guidedPreview.complete || (globallyBusy && busyAction !== "migration-apply")}>{outcome?.kind === "needs-attention" ? "REVIEW STOPPED RUN" : outcome?.kind === "guided" ? "GUIDED HANDOFF PREPARED" : outcome?.kind === "simulated" ? "FIXTURE PLAN SIMULATED" : outcome?.kind === "aligned" ? "ALREADY ALIGNED" : outcome?.kind === "live-applied" ? "CONNECTED ACTIONS APPLIED" : outcome ? "LOCAL ACTIONS APPLIED" : guidedPreview.requiresExactReview ? "PREPARE GUIDED HANDOFF" : "APPLY TRANSLATED PLAN"}</ActionButton>{outcome ? <p className="success-note">{outcome.message}</p> : null}</div>
...
export function CompanionSpread({
  ...
  return (
    <section className="passport-book section-book">
      <div className="book-spine" aria-hidden="true" />
      <article className="passport-page left-page">
        <PageHeading eyebrow="FIRST PERSON · SELECTIVE SHARING" title="Companion Invitation" note="The first person shares only their selected fields. This step cannot activate a blend." page="11" />
      ...
      <article className="passport-page right-page">
        <PageHeading eyebrow={companion ? companion.code : invitation ? "SECOND PERSON CHECKPOINT" : "AWAITING INVITATION"} title="Two-Person Blend Controls" note="The second person chooses their own fields separately. Both people must act before the reversible overlay can start." page="12" />
