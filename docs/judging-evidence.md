...
| Video | Public YouTube or Vimeo; maximum five minutes; working demo plus problem, user, and importance | An approximately 185-second clean-revision tutorial cut and matching voiceover script show the complete before/request/agent/result flow; record or supply the voiceover, then publish the narrated cut |
...
| Presentation | 20% | The approximately 185-second tutorial pans through archived real YouTube and Bluesky dummy-account feeds, labels visible content, demonstrates conversational and exact requests, shows a separate managed AWS check, measures a local practice plan, places real feeds side by side before/after, and completes Copy Feed, Incognito, and two-person Blend flows; the custom-domain site and judge login are live | Narration, public YouTube/Vimeo upload, Devpost screenshots, and final submission copy remain entrant-owned finishing work |
...
| Silent demo video | Ready to regenerate from the release commit at 1440×900 once the four reviewed dummy-account captures and retained redacted managed receipts are present; the recorder binds the clean Git revision and evidence digest, checks both prompt styles, verifies four scrolling tours, two direct before/after comparisons, three cards per platform in each Curate comparison, Copy Feed result, Incognito issue/revoke, Blend invitation/activation/stop, managed-proof interstitial, and rollback, and rejects external browser requests | Add the recorded voiceover when supplied, then upload the final narrated cut to YouTube or Vimeo |
