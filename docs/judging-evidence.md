...
| AgentCore | Optional; a live demo and/or AgentCore deployment strengthens Technical Implementation | The checkout has a proposal-only Strands/Bedrock Runtime package plus plan-first CDK. It exposes strict `health`/`plan_feature` commands and no mutation authority. Claim deployment only after separate entrant approval and a deployment/invocation receipt |
...
| Live AgentCore deployment | Prepared but not deployed; optional and unnecessary for the local proof | Under zero spend, run only local package/synth validation. Add a live claim only after explicit entrant approval and deployment/invocation receipts; never imply credits guarantee zero cost |
| External live accounts | Candidate transports are source- and contract-tested; no account/receipt proof exists | Never imply an OAuth registration, connection, or Guided plan was executed. Add only fresh authorized-dummy-account conformance receipts and keep unsupported platforms Guided |
