  ...
  return (
    <section className="passport-book section-book">
      ...
      <article className="passport-page right-page">
        <PageHeading eyebrow="PROPOSAL, NOT SILENT ACTION" title="Correction Notice" note="The agent explains the smallest change and waits for explicit approval." page="14" />
        {canCorrect || correctionApplied || correctionSimulated ? <><article className="correction-notice"><StatusStamp tone={correctionApplied ? "green" : correctionSimulated ? "blue" : "orange"}>{correctionApplied ? "Receipted" : correctionSimulated ? "Simulated" : "Attention"}</StatusStamp><h3>{correctionApplied ? "The approved Lab correction was receipted" : correctionSimulated ? "The fixture correction plan was simulated" : "Repeated creators are crowding out useful surprise"}</h3><p>{drift.recommendation || "Add three independent source candidates and reduce repeated creator weight before touching topic proportions."}</p><dl><div><dt>Constitution change</dt><dd>None</dd></div><div><dt>Proposed operation</dt><dd>Source-diversity overlay</dd></div><div><dt>Execution scope</dt><dd>{correctionSimulated ? "Deterministic fixture only" : "Feed Passport Lab"}</dd></div><div><dt>Rollback</dt><dd>{correctionSimulated ? "Not required" : "Exact checkpoint"}</dd></div></dl></article><div className="before-after"><div><span>BEFORE</span><b>18</b><small>creator repetition</small></div><div><span>{correctionApplied ? "RECEIPTED" : correctionSimulated ? "SIMULATED" : "PROPOSED"}</span><b>13</b><small>creator repetition</small></div></div></> : <article className="correction-notice"><StatusStamp tone="blue">No active proposal</StatusStamp><h3>A fresh decision-required drift check is needed</h3><p>{drift.recommendation || "Run a fresh Lab observation before approving any corrective controls."}</p><dl><div><dt>Constitution change</dt><dd>None</dd></div><div><dt>Proposed operation</dt><dd>None active</dd></div><div><dt>Execution scope</dt><dd>Feed Passport Lab</dd></div><div><dt>Rollback</dt><dd>Not required</dd></div></dl></article>}
        <ActionButton onClick={onCorrect} busy={busy} disabled={correctionApplied || correctionSimulated || !canCorrect}>{correctionApplied ? "CORRECTION RECEIPTED" : correctionSimulated ? "FIXTURE CORRECTION SIMULATED" : canCorrect ? "APPROVE LAB CORRECTION" : "RUN FRESH CHECK FIRST"}</ActionButton>
        ...
        <section className="monitor-office">
          ...
          {monitor?.status === "active" ? (
            ...
            <>
              ...
              <ActionButton variant="ink" onClick={onCreateMonitor} busy={busy}>{monitorConfig.mode === "bounded_auto" ? "APPROVE BOUNDED LAB MONITOR" : "START ALERT-ONLY MONITOR"}</ActionButton>
            </>
          )}
          <small>{monitorConfig.mode === "bounded_auto" ? "This click explicitly authorizes certified Lab controls only, three actions per run, with an immediate stop." : "Alert-only mode may observe and notify, but cannot execute a correction."} Model-facing agents can schedule alert-only monitoring only.</small>
        </section>
  ...
  return (
    <section className="passport-book section-book">
      ...
      <article className="passport-page right-page receipt-detail-page">
        ...
        {selected ? <article className="receipt-detail"><div className="receipt-detail-stamp"><span>AUDIT</span><b>{selected.status.toUpperCase()}</b><small>{selected.time}</small></div><h3>{selected.type}</h3><p>{selected.detail}</p><dl><div><dt>Receipt ID</dt><dd>{selected.id}</dd></div><div><dt>Checkpoint</dt><dd>{selected.checkpoint}</dd></div><div><dt>Credentials included</dt><dd>No</dd></div><div><dt>Raw private content</dt><dd>No</dd></div><div><dt>Reversible</dt><dd>{selected.reversible ? "Yes" : "No"}</dd></div></dl>{selected.reversible ? <ActionButton variant="danger" onClick={() => onRollback(selected)} busy={busyAction === "rollback"} disabled={Boolean(busyAction) && busyAction !== "rollback"}>ROLL BACK THIS RECEIPT</ActionButton> : <StatusStamp tone="blue">No rollback required</StatusStamp>}</article> : null}
