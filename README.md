...
Open `http://localhost:5173`. With no service base configured, the placard and Practice Run page show **Practice mode**. This is the zero-credential product walkthrough.

The **Practice Run** page is the account-free autonomy proof. Choose one of ten platform-shaped practice feeds, describe the outcome, set the change and pass limits, and choose the target match. **MAKE A PRACTICE PLAN** uses the configured Strands planner; **QUICK PREVIEW** uses the same checked route without model interpretation. Both show the starting feed and expected result before the run button appears.

The global **Ask Curate** page uses the same loopback-only provider to choose one migration, temporary-feed, or shared-view route. The app creates the visible explanation and fills the matching page; the person still reviews and starts the feature there.
