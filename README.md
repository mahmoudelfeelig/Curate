...
Feed Passport is an agentic control layer for recommendation feeds. A person writes the feed they want as a portable, versioned policy; the agent translates that intent into only the controls a destination declares, asks for bounded approval, re-observes after acting, and stops with a rollback receipt. The complete loop runs locally in Feed Passport Lab and ten account-free platform control twins. Real external destinations remain Guided in the current evidence set; four bounded live candidates are implemented but require owner authorization and signed dummy-account conformance before they can even be loaded, and generic migrations cannot execute them.
...
- This implementation pass created no social account, completed no external OAuth flow, contacted no social API, deployed no AWS resource, and invoked no AgentCore Runtime, Bedrock model, or other billable provider. Those claim levels remain pending until separate user-owned setup and receipts exist.
...
- A proposal-only Strands feature planner and hardened Amazon Bedrock AgentCore Runtime package are implemented with a plan-first CDK stack. The strict `plan_feature` command accepts one bounded request, while arbitrary chat/unstructured commands and every execute/approve/rollback operation are rejected; there is no deployment or managed invocation proof in this repository.
...
| Migration | Compiles destination actions, declares translation loss, requires an approval envelope, executes against Lab/twins, or opens an exact human handoff where only native controls exist | Closed loop in Feed Passport Lab; structured missions against ten local twins; owner-bound guided-handoff state and receipts; candidate transports/recovery journals for YouTube, X, Reddit, and Bluesky | Generic migration authority cannot execute a live adapter; the unrouted owner-authenticated commission boundary is the only implemented live execution authority, and no external action is certified or validated here |
...
| Agent control | One request-bound Strands clerk proposes migration, temporary-visa, or companion parameters; a second mission planner may narrow a selected twin's controls; deterministic policy remains authoritative | Both local planners enforce an exact three-tool proposal protocol, strict typed output, positive token evidence, loopback-only configuration, and adversarial authority tests; the AgentCore package adds a proposal-only Bedrock path with scripted no-network tests | No model can choose identity or account resources, handle credentials, create partner consent, widen budgets or thresholds, approve itself, execute, or roll back |
...
| Bluesky | Guided; live candidate unpromoted | Official AT Protocol OAuth/DPoP sidecar for follows/unfollows, actor mute/unmute, and muted-word add/remove; internal commission seam | Needs durable HTTPS sidecar deployment, authorized dummy account, and signed conformance receipt; the commission has no public route; custom feeds and ranking state are not copied |
...
| YouTube | Guided; live candidate unpromoted | OAuth/Data API subscription list/insert/delete; internal commission seam | Needs authorized dummy account and signed conformance receipt; the commission has no public route; Home ranking, watch history, and native recommendation feedback are neither read nor written |
