...
class CuratorApplication:
    ...
    def execute_migration(
        ...
        *,
        ...
        if migration["status"] not in {"awaiting_approval", "failed_recoverable"}:
        ...
        if passport.owner_id != approved_by:
        ...
        receipt_projection = {**to_primitive(receipt), "platform": platform}
        self._record(
            kind="receipts",
        ...
        self._persist_adapter_state(platform, account_id, approved_by, trace_id)
        migration.update(
            {
                "status": "issued" if evaluation.stop_reason is StopReason.TARGET_REACHED else "issued_with_drift",
        ...
        self._record(
            ...
            payload={"receipt_id": receipt.id, "stop_reason": evaluation.stop_reason.value},
            actor_id=approved_by,
            trace_id=trace_id,
        )
        ...
        if overlay_id:
            ...
            overlay["activations"] = activations
            self._record(
                kind="overlays",
                ...
                projection=overlay,
                payload={"migration_id": migration_id, "receipt_id": receipt.id, "platform": platform},
                actor_id=approved_by,
                trace_id=trace_id,
            )
        return migration
    ...
    def rollback_receipt(self, receipt_id: str, *, actor_id: str, platform: str) -> dict[str, Any]:
        ...
        outcome = adapter.rollback(receipt.destination_id, receipt, now=self._now())
        if outcome.failed_actions:
            rollback_status = "rollback_partial" if outcome.restored_actions else "rollback_failed"
        else:
    ...
    def _persist_adapter_state(self, platform: str, account_id: str, actor_id: str, trace_id: str) -> None:
        ...
        self._record(
            ...
            projection={"platform": platform, "account_id": account_id, "state": projection},
