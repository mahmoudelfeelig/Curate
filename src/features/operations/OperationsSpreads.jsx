  ...
  return (
    <section className="passport-book section-book">
      ...
      <article className="passport-page right-page">
        ...
        {canCorrect || correctionApplied || correctionSimulated ? <><article className="correction-notice"><StatusStamp tone={correctionApplied ? "green" : correctionSimulated ? "blue" : "orange"}>{correctionApplied ? "Recorded" : correctionSimulated ? "Previewed" : "Attention"}</StatusStamp><h3>{correctionApplied ? "The feed correction is on the record" : correctionSimulated ? "The correction was tried in practice mode" : "Repeated creators are crowding out useful surprise"}</h3><p>{drift.recommendation || "Add three independent source candidates and reduce repeated creator weight before touching topic proportions."}</p><dl><div><dt>Your mix</dt><dd>Unchanged</dd></div><div><dt>Suggested change</dt><dd>More source variety</dd></div><div><dt>Where it runs</dt><dd>{correctionSimulated ? "Practice feed" : "Curate Lab"}</dd></div><div><dt>Return route</dt><dd>{correctionSimulated ? "No change made" : "Starting checkpoint"}</dd></div></dl></article><div className="before-after"><div><span>BEFORE</span><b>18</b><small>creator repetition</small></div><div><span>{correctionApplied ? "RECORDED" : correctionSimulated ? "PREVIEWED" : "PROPOSED"}</span><b>13</b><small>creator repetition</small></div></div></> : <article className="correction-notice"><StatusStamp tone="blue">No active suggestion</StatusStamp><h3>Check the feed again before changing it</h3><p>{drift.recommendation || "Run a fresh Curate check before applying a correction."}</p><dl><div><dt>Your mix</dt><dd>Unchanged</dd></div><div><dt>Suggested change</dt><dd>None yet</dd></div><div><dt>Where it runs</dt><dd>Curate Lab</dd></div><div><dt>Return route</dt><dd>No change made</dd></div></dl></article>}
        <ActionButton onClick={onCorrect} busy={busy} disabled={correctionApplied || correctionSimulated || !canCorrect}>{correctionApplied ? "CORRECTION RECORDED" : correctionSimulated ? "PRACTICE CORRECTION PREVIEWED" : canCorrect ? "TRY THE CORRECTION" : "CHECK THE FEED FIRST"}</ActionButton>
  ...
  return (
    <section className="passport-book section-book">
      ...
      <article className="passport-page left-page">
        ...
        <Field label="Search the creator directory"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, field, or platform" /></Field>
      ...
      <article className="passport-page right-page">
        ...
        <div className="continuity-route">{creators.filter((creator) => preserved.includes(creator.id)).map((creator) => <article key={creator.id}><div><span>{creator.source}</span><b>{creator.name}</b></div><div className="route-divider">IDENTITY MATCH</div><div><span>{creator.destination}</span><b>{creator.destinationHandle}</b></div><StatusStamp tone={creator.confidence >= 90 ? "green" : "orange"} compact>{creator.confidence}% confidence</StatusStamp></article>)}{!preserved.length ? <div className="empty-manifest"><b>NO MATCHES PRESERVED</b><p>Select a reviewed creator from the facing page.</p></div> : null}</div>
        <aside className="border-note"><strong>Identity safety</strong><p>The practice directory uses reviewed public links. Curate never guesses from private contacts or silently follows a match.</p></aside>
