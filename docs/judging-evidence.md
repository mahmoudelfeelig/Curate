...
| AgentCore | Optional; a live demo and/or AgentCore deployment strengthens Technical Implementation | The checkout keeps a deterministic-command compatibility entrypoint and disables broad provider-backed free text. Claim deployment only after separate entrant approval and a deployment receipt |
| Repository | Public GitHub, GitLab, or Bitbucket repository with source, assets, setup instructions, and README | Public visibility remains an external submission check |
| License | MIT or Apache license, detectable and visible in repository metadata/About | MIT file exists; confirm host detection after publication |
...
| Third-party integrations | SDKs, APIs, and data require authorization and license compliance | Current external adapters make no calls; add live integrations only with approved access and conformance evidence |
...
| Strands viability | A request-bound `strands.Agent` uses a pinned Qwen3 1.7B model through the official llama.cpp b8184 loopback runtime. The recorded browser flow called exactly `inspect_selected_passport`, `inspect_selected_control_surface`, and `submit_mission_proposal`, used 3,521 tokens, passed deterministic validation, and stopped awaiting consent with zero paid or external model calls | **Local model** proves genuine inference, not AWS/AgentCore deployment or access to any social account |
...
| --- | ---: | --- | --- |
| Technical Implementation | 20% | Two request-bound Strands protocols through a pinned loopback runtime; exact tool traces and token accounting; typed Feature Clerk submissions with no model-authored prose; capability-bound deterministic rendering; deterministic owner checks, proposal admission, and mission-bound one-time consent; model-facing context excludes identity, credentials, approval, execution, and rollback authority; persisted missions, budgets, evaluations, stop reasons, and receipts; ten restricted local control twins; exact reverse-order rollback verification; SQLite events/projections and atomically claimed jobs; autonomous FastAPI lifespan runner; fail-closed AgentCore compatibility entrypoint; fresh final test counts are recorded only after the integrated verification run | No authenticated identity middleware, AgentCore deployment receipt, external credentials/transport, or authorized external-platform conformance receipt. Those are distinct future proofs, not local-agent gaps |
| Design | 20% | A responsive utility built around a coherent 2009–2013 passport/customs metaphor; Feature Clerk makes model classification inspectable through its exact trace, tokens, latency, server-rendered text source, bound capability, and proposal-only apply behavior; Agent mission makes bounded autonomy inspectable through hard budgets, consent, measured execution, stop reason, and rollback | Final desktop/mobile evidence includes fixture and service workflows, keyboard/history/reduced-motion checks, and console/network/overflow/clipping assertions; no external user-study or full WCAG-conformance claim is included |
...
| External capability claims are conservative | `adapters/platforms/base.py`, ten platform profiles, and `tests/test_platform_adapters.py`; unprefixed adapters remain credential-free guided planners |
...
| AgentCore is implemented but not proven deployed | `runtime/agentcore_app.py` and its tests; no deployment output is checked in |
...
| WebMCP is bounded to this site | `src/webmcp.js` and `src/webmcp.test.mjs`; six tools are registered, including local-mission preview and inspection, while tests assert that no approval, execution, or run-mission tool exists |
...
The committed [evidence manifest](../artifacts/evidence/manifest.json) binds each curated report and screenshot to its path and SHA-256, plus a canonical digest of the tested source snapshot excluding the self-referential manifest file.

## Submission readiness

| Item | Status on August 31 | Required before submission |
| --- | --- | --- |
...
| Deterministic evaluation | Verified: 14 of 14 scenarios passed | Retain the curated JSON used for the recording |
| Account-free local agent mission | Verified in fixture, deterministic-service, and genuine local-model Chrome flows | Record the already verified three-tool plan, deterministic scope reduction, scoped approval, bounded run, honest `no_actionable_plan` stop, receipt, and exact fingerprint restoration on a `twin:<platform>` without implying ranking fidelity |
| Feature Clerk local-model proof | Self-owned pinned-runtime harness covers migration, Temporary Visa, and Companion with adversarial identity, authority, and capability prose | Retain two cold-start receipts plus the final rendered browser flow; applying a result must only pre-fill a deterministic desk and perform no mutation call |
| Local model runtime | Verified with Qwen3 1.7B and official llama.cpp b8184 on loopback; Feature Clerk used 5,491 tokens and the mission planner used 3,521 in the final browser flow, with zero paid/external model calls | Keep the pinned hashes and local-model evidence visible; do not substitute an unverified hosted provider during recording |
| Public repository | Not verified | Publish with all judged source/assets/setup instructions |
...
| Live AgentCore deployment | Optional, unverified, and unnecessary for the local proof | Add only after explicit entrant approval and a deployment/invocation receipt; never imply credits were used otherwise |
| External live accounts | Not part of current proof | Never imply guided plans were executed; add only authorized conformance receipts |
