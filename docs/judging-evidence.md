...
| Video | Public YouTube or Vimeo; maximum five minutes; working demo plus problem, user, and importance | An approximately 108-second clean-revision silent cut and matching short voiceover script are ready once the four reviewed platform captures are recorded; record/mix the supplied voiceover, then publish the narrated cut |
...
| Presentation | 20% | The approximately 108-second cut opens on archived real YouTube and Bluesky dummy-account feeds, demonstrates conversational and exact requests, shows the real feeds and Curate comparison before/after, a local-model plan and rollback, Copy feed, Incognito, and Blend; the custom-domain site and judge login are live | Narration, public YouTube/Vimeo upload, Devpost screenshots, and final submission copy remain entrant-owned finishing work |
...
| Silent demo video | Ready to regenerate from the release commit at 1440×900 once the four reviewed dummy-account captures are present; the recorder binds the clean Git revision and evidence digest, checks both prompt styles, verifies the real YouTube and Bluesky before/after frames, three cards per platform in each Curate comparison, Copy feed, Incognito issue/revoke, Blend, and rollback, and rejects external browser requests | Add the recorded voiceover when supplied, then upload the final narrated cut to YouTube or Vimeo |
