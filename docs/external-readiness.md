...
Package and full-plan commands intentionally fail while the Git worktree is dirty. Once the intended source is committed, they embed and validate the exact clean `HEAD`; `-SkipPackage` cannot reuse an artifact from another commit. The current Python image tag and ranged application dependencies are mutable, so this is source provenance and deterministic ZIP serialization, not byte-for-byte dependency reproducibility.
